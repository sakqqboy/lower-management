	...
	<div class="col-12 mt-20">
		<div class="row">
			...
			<div class="col-2 pt-10">
				<input type="checkbox" class="checkbox-sm" id="report" <?= $report == 1 ? 'checked' : '' ?>> Business report jobs
			</div>
			<div class="col-2 pt-10">
				<div class="row">
			...
			<div class="col-4 text-right pt-10">
				<span class="box-sort" onclick="javascript:sortStepDue()">Step due
					<img src="<?= Yii::$app->homeUrl ?>images/icon/maxmin.png" class="sort-icon ml-10" id="maxmin-step" style="display:<?= $showMaxminStep ?>;">
					<img src="<?= Yii::$app->homeUrl ?>images/icon/minmax.png" class="sort-icon ml-10" id="minmax-step" style="display:<?= $showMinmaxStep ?>;">
					<input type="hidden" id="sort-step" value="<?= $postSortStep ?>">
				</span>
				<span class="box-sort ml-20" onclick="javascript:sortFinalDue()">Final due
					<img src="<?= Yii::$app->homeUrl ?>images/icon/maxmin.png" id="maxmin-final" class="sort-icon ml-10" style="display:<?= $showMaxminFinal ?>;">
					<img src="<?= Yii::$app->homeUrl ?>images/icon/minmax.png" id="minmax-final" class="sort-icon ml-10" style="display:<?= $showMinmaxFinal ?>;">
					<input type="hidden" id="sort-final" value="<?= $postSortFinal ?>">
				</span>
				<a href="javascript:exportJob()" class="btn button-turqouise no-underline ml-20"><i class="fa fa-print" aria-hidden="true"></i> Export</a>
		...
		$gridColumns = [
			...
			[
				...
				'value' => function ($model) {
					...
					//$text = "<div class='row'><div class='col-12 text-left border'>" . Step::stepNameByJsId($model->jsJobStepId) . "</div>" . "<div class='col-12 text-right border'>" . $model->dueDate != null && $model->dueDate != '' ? ModelMaster::engDate($model->dueDate, 2) : 'not set due date' . '</div></div>';
					//$stepName = Step::stepNameByJsId($model->jsJobStepId);
					//$dueDate = "<div class='col-12 text-right border'>" . $model->dueDate != null && $model->dueDate != '' ? ModelMaster::engDate($model->dueDate, 2) : 'not set due date' . "</div>";
					//return  '=>' . $model->dueDate;
					//return   '<div class="row">' . $stepName . $dueDate . '</div>';
					//return $stepName . '=>' . $model->jsJobStepId . '=>' . $model->jsStepId . '=>' . $model->jsStatus . '=>' . $model->maxjs;
				},
